...
* **Resource Pages**: Located in `.agents/wiki/resources/<page_name>.md`
* **Pattern Pages**: Located in `.agents/wiki/patterns/<page_name>.md`
* **Gotcha Pages**: Located in `.agents/wiki/gotchas/<page_name>.md`

### Mandatory (Always Read — 2 pages)
- `.agents/wiki/improvements/naming-conventions.md`
- `.agents/wiki/improvements/code-improvement-checklist.md`
...
Read the FULL improvement checklist from: `.agents/wiki/improvements/code-improvement-checklist.md`
...
- **Azure Naming Compliance (Self-Check):** Azure enforces strict, per-resource naming rules (length, charset, global uniqueness) that `terraform validate` cannot catch — they only fail at apply time. After writing your category's files, self-check names by running the `azure-naming-validator` skill and fix any `error`-severity findings before returning:
  `python3 .agents/skills/azure-naming-validator/run.py --dir . --output output/artifacts/azure-naming-results.json`

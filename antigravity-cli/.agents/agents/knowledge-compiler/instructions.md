...
- Compile and update the entire local `.agents/wiki/` resources, patterns, and gotchas.
...
2. **Execute Targeted Fetch:** Do NOT perform a full scan. Run a single focused MCP/fetch query targeting the requested resource or search query:
   - For `docs`: Get the exact specifications and code examples for the requested resource and write to `.agents/wiki/resources/<resource_name>.md`.
   - For `gotcha`: Perform a live Web Search query (via MCP/fetch) for the error message/SKU issue, summarize the resolution, and append to `.agents/wiki/gotchas/<resource_name>_fix.md`.
...
- Read from: `.agents/wiki/` — existing wiki pages to update
...
## Output
- Write entity pages to: `.agents/wiki/resources/`
- Write pattern pages to: `.agents/wiki/patterns/`
- Write gotcha pages to: `.agents/wiki/gotchas/`

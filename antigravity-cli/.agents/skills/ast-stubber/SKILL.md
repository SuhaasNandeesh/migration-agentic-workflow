...
```bash
python3 .agents/skills/ast-stubber/run.py --file /path/to/large/file.tf --stub --output output/artifacts/stubs/large/file.tf
```
...
```bash
python3 .agents/skills/ast-stubber/run.py --file /path/to/large/file.tf --hydrate --line-range 120-165
```

Alternatively, hydrate by specific block name or identifier:

```bash
python3 .agents/skills/ast-stubber/run.py --file /path/to/large/file.tf --hydrate --block-name aws_db_instance.primary
```

...
After writing memory entries, UPDATE the Knowledge Wiki at `.claude/wiki/`:

### 1. Update Entity Pages
For each resource type that was migrated in this run:
- Read the existing entity page in `.claude/wiki/resources/`
...
- Check if a gotcha page already exists in `.claude/wiki/gotchas/`
...
- Check if a pattern page exists in `.claude/wiki/patterns/`

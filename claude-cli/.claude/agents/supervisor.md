...
2. **Wiki Caching & Compiler compilation**:
   - Check if `.claude/wiki/index.md` exists
  ...
  For each category in wave.categories:
    ...
    1. Invoke developer (SEMANTIC PROMPT ASSEMBLY):
       - Task: "Generate code for ONLY the {category} category.
               ...
               - `.claude/wiki/resources/azurerm_{category_resource}.md`
               - `.claude/wiki/gotchas/azurerm_{category_resource}_fix.md` (if exists)
               - `.claude/wiki/patterns/{category_pattern}.md` (if exists)
    ...
    4. If reviewer FAILS (PROGRESSIVE RETRY COMPACTION):
       ...
       - For Attempt 3 (SURGICAL-FIX DYNAMIC MCP ESCALATION):
         ...
         → `knowledge-compiler` queries live MCP/Web, writes fix to `.claude/wiki/gotchas/azurerm_{resource}_fix.md` and deletes `mcp_request.json`
...
All agents should reference the Knowledge Wiki at `.claude/wiki/` for:
- Resource entity pages → `.claude/wiki/resources/`
- Migration patterns → `.claude/wiki/patterns/`
- Known gotchas → `.claude/wiki/gotchas/`
- Code improvement rules → `.claude/wiki/improvements/`

When delegating to `developer`, tell it: "Read improvement patterns from .claude/wiki/improvements/code-improvement-checklist.md"
When delegating to `code-reviewer`, tell it: "Verify improvements against .claude/wiki/improvements/code-improvement-checklist.md"

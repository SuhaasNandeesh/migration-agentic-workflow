...
To invoke a subagent autonomously, you MUST use the `Bash` tool to run the Claude CLI (e.g., `claude --agent code-reviewer "Please review the generated files"`).
To utilize a skill/command, invoke the custom slash command `/<skill-name>` natively or run `claude /<skill-name>` via the Bash tool.

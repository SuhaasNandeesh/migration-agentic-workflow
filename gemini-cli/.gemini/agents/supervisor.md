  ...
  For each category in wave.categories:
    ...
    1. Invoke developer (SEMANTIC PROMPT ASSEMBLY):
       - Task: "Generate code for ONLY the {category} category.
               ...
               - `.gemini/wiki/resources/azurerm_{category_resource}.md`
               - `.gemini/wiki/gotchas/azurerm_{category_resource}_fix.md` (if exists)
               - `.gemini/wiki/patterns/{category_pattern}.md` (if exists)
    ...
    4. If reviewer FAILS (PROGRESSIVE RETRY COMPACTION):
       ...
       - For Attempt 3 (SURGICAL-FIX DYNAMIC MCP ESCALATION):
         ...
         → `knowledge-compiler` queries live MCP/Web, writes fix to `.gemini/wiki/gotchas/azurerm_{resource}_fix.md` and deletes `mcp_request.json`
...
All agents should reference the Knowledge Wiki at `.gemini/wiki/` for:
- Resource entity pages → `.gemini/wiki/resources/`
- Migration patterns → `.gemini/wiki/patterns/`
- Known gotchas → `.gemini/wiki/gotchas/`
- Code improvement rules → `.gemini/wiki/improvements/`

When delegating to `developer`, tell it: "Read improvement patterns from .gemini/wiki/improvements/code-improvement-checklist.md"
When delegating to `code-reviewer`, tell it: "Verify improvements against .gemini/wiki/improvements/code-improvement-checklist.md"

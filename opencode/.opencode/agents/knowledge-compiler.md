...
2. **Execute Targeted Fetch:** Do NOT perform a full scan. Run a single focused MCP/fetch query targeting the requested resource or search query:
   - For `docs`: Get the exact specifications and code examples for the requested resource and write to `.opencode/wiki/resources/<resource_name>.md`.
   - For `gotcha`: Perform a live Web Search query (via MCP/fetch) for the error message/SKU issue, summarize the resolution, and append to `.opencode/wiki/gotchas/<resource_name>_fix.md`.

  ...
  For each category in wave.categories:
    ...
    1. Invoke developer (SEMANTIC PROMPT ASSEMBLY):
       - Task: "Generate code for ONLY the {category} category.
               ...
               - `.opencode/wiki/resources/azurerm_{category_resource}.md`
               - `.opencode/wiki/gotchas/azurerm_{category_resource}_fix.md` (if exists)
               - `.opencode/wiki/patterns/{category_pattern}.md` (if exists)
    ...
    4. If reviewer FAILS (PROGRESSIVE RETRY COMPACTION):
       ...
       - For Attempt 3 (SURGICAL-FIX DYNAMIC MCP ESCALATION):
         ...
         → `knowledge-compiler` queries live MCP/Web, writes fix to `.opencode/wiki/gotchas/azurerm_{resource}_fix.md` and deletes `mcp_request.json`

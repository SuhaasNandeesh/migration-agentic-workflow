...
2. **Wiki Caching & Compiler compilation**:
   - Check if `.pi/wiki/index.md` exists
  ...
  For each category in wave.categories:
    ...
    1. Invoke developer (SEMANTIC PROMPT ASSEMBLY):
       - Task: "Generate code for ONLY the {category} category.
               ...
               - `.pi/wiki/resources/azurerm_{category_resource}.md`
               - `.pi/wiki/gotchas/azurerm_{category_resource}_fix.md` (if exists)
               - `.pi/wiki/patterns/{category_pattern}.md` (if exists)
    ...
    4. If reviewer FAILS (PROGRESSIVE RETRY COMPACTION):
       ...
       - For Attempt 3 (SURGICAL-FIX DYNAMIC MCP ESCALATION):
         ...
         → `knowledge-compiler` queries live MCP/Web, writes fix to `.pi/wiki/gotchas/azurerm_{resource}_fix.md` and deletes `mcp_request.json`
